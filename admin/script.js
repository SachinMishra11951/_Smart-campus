// Feature: Application Initialization and Authentication Verification
...
// Feature: Dashboard Statistics Tracking and DOM Element Updating
function updateAdminStats() {
    ...
    const bookBadge = document.getElementById('admin-sidebar-booking-badge');
    
    if (bookBadge) {
        bookBadge.textContent = pendingBookings;
        bookBadge.style.display = pendingBookings > 0 ? 'inline-block' : 'none';
    }
}

// Feature: Admin Data Retrieval APIs for Complaints, Bookings, Users, and Resources
async function fetchAllComplaints() {
    try {
        ...
        updateAdminStats();
        updateComplaintVisualizations(allComplaints);
    } catch (err) {
...
async function fetchAllResources() {
    try {
        ...
        const resList = data.resources || data || [];
        renderAdminResources(resList);
    } catch (err) {
        console.error("Resources Fetch Error:", err);
    }
}

// Feature: Cosmetic UI Scripts for Stars Background and Theme Toggling
...
// Feature: Application DOM Navigation and Modal State Handlers
const pageTitles = {
    dashboard: "DASHBOARD",
...
// Feature: Notification Handlers and Read State Updaters
...
// Feature: Admin Action Handlers for System Entities
...
function approveBooking(id) { updateBookingStatus(id, "approved"); }
function rejectBooking(id) { updateBookingStatus(id, "rejected"); }
...
// Feature: Complaints Datatable View and Filter Logic
function statusPill(s) {
    const val = (s || 'open').toLowerCase();
    const map = { 
        open: 'status-open', pending: 'status-open', 
        assigned: 'status-assigned', in_progress: 'status-assigned',
        resolved: 'status-resolved', approved: 'status-resolved',
        escalated: 'status-escalated', rejected: 'status-escalated', 
        closed: 'status-closed' 
    };
    const displayStatus = s ? (s.charAt(0).toUpperCase() + s.slice(1)) : 'Open';
    return `<span class="status-pill ${map[val] || 'status-open'}">${displayStatus}</span>`;
}
...
function filterComplaints() {
    ...
    const filtered = allComplaints.filter(c => {
        return (!search || c.id.toString().includes(search) || (c.title && c.title.toLowerCase().includes(search)))
            && (!cat || c.category === cat)
            && (!status || (c.status && c.status.toLowerCase() === status.toLowerCase()))
            && (!priority || (c.priority && c.priority.toLowerCase() === priority.toLowerCase()));
    });
    
    renderComplaintsTable(filtered);
    const countText = document.getElementById('complaint-type-text');
    if (countText) countText.textContent = `Showing ${filtered.length} result(s)`;
}
...
function escalateComplaint(id) {
    ...
    if (c) { c.status = 'Escalated'; renderComplaintsTable(allComplaints); }
}

function closeComplaint(id) {
    const c = allComplaints.find(x => x.id === id);
    if (c && confirm(`Close complaint ${id}?`)) { c.status = 'Closed'; renderComplaintsTable(allComplaints); }
}

function changePage(dir) { alert(dir > 0 ? 'Next page' : 'Previous page'); }

// Feature: Dynamic Data Renderers for Bookings and Users
...
// Feature: Graphical Dashboard Components
const progressData = [
...
function renderProgressBars() {
    ...
    setTimeout(() => {
        document.querySelectorAll('.progress-fill[data-w]').forEach(el => { el.style.width = el.getAttribute('data-w') + '%'; });
    }, 300);
}
...
function startLiveFeed() {
    ...
    function type() {
        ...
        if (charIdx <= msg.length) {
            ...
            setTimeout(() => { charIdx = 0; liveIdx++; type(); }, 2800);
        }
    }
    type();
}
...
// Feature: Canvas-based Chart Generation Functions
function initCharts() {
    drawTrendChart();
...
function drawDynamicStatsPie(slices) {
...
function updateComplaintVisualizations(complaintsData) {
    ...
    const colors = ['#6366f1', '#22d3ee', '#fbbf24', '#f472b6', '#4ade80', '#f87171'];
    
    const slices = Object.entries(categoryCounts).map(([label, count], index) => ({
        l: label,
        v: Math.round((count / total) * 100),
        c: colors[index % colors.length]
    }));

    const donutCenter = document.querySelector('.donut-center span');
    ...
    if (legendContainer) {
        legendContainer.innerHTML = slices.map(s => `
            <div class="legend-item">
                <div class="legend-dot" style="background:${s.c}"></div>${s.l} (${s.v}%)
            </div>
        `).join('');
    }
    ...
    if (donutChart) {
        ...
        let currentAngle = 0;
        slices.forEach(s => {
            let angle = (s.v / 100) * 360;
            gradientString.push(`${s.c} ${currentAngle}deg ${currentAngle + angle}deg`);
            currentAngle += angle;
        });
        donutChart.style.background = `conic-gradient(${gradientString.join(', ')})`;
    }

    drawDynamicStatsPie(slices);
}

// Feature: Form Utility Functions and User Registration Flow
...
// Feature: Global Header Search Handling
document.getElementById('global-search').addEventListener('input', function () {
    ...
    if (q.includes('complaint')) {
        showPage('complaints', document.querySelector('[onclick*="complaints"]'));
    } else if (q.includes('book') || q.includes('approval')) {
        showPage('approvals', document.querySelector('[onclick*="approvals"]'));
    } else if (q.includes('resource')) {
        showPage('resources', document.querySelector('[onclick*="resources"]'));
    } else if (q.includes('user')) {
        showPage('users', document.querySelector('[onclick*="users"]'));
    } else if (q.includes('stat') || q.includes('analytic')) {
        showPage('statistics', document.querySelector('[onclick*="statistics"]'));
    } else if (q.includes('notif')) {
